...
/**
 * Reconstruct the Artist→Album→Track tree from raw Drive file lists.
 * This is a pure function (no API calls) used by both full scan and incremental sync.
 */
export function buildLibraryTree(
  rootId: string,
  allFolders: DriveFile[],
  allAudioFilesRaw: DriveFile[],
  allCoverFiles: DriveFile[],
): Omit<ScanResult, "allFolders" | "allAudioFiles" | "allCoverFiles"> {
  ...
  for (const sf of albumSubfolderList) {
    ...
    if (filesInSf.length === 0) continue;
...
export async function scanLibrary(
  ...
  const tree = buildLibraryTree(rootId, allFolders, allAudioFilesRaw, allCoverFiles);

  // allAudioFiles in the result should be the filtered set (actual audio files only)
  const allAudioFiles = allAudioFilesRaw.filter((f) => isAudioFile(f));
